...
4. **UI & HUD**: UI overlays read the active aircraft's local property bus for per-aircraft telemetry. The UI remains data-driven, but the source of truth is the active `Aircraft::Instance` rather than the global bus.
