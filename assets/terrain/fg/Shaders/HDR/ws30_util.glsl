...
/*
 * Generates a full precision 32 bit random number from 2 seeds as well as
 * 6 random integers between 0 and factor that are rescaled 0.0-1.0 by
 * re-using the significant figures from the full precision number.
 *
 * This avoids having to generate 6 random numbers when limited variation is
 * needed: say 6 numbers with 100 levels (i.e between 0 and 100).
 *
 ...
 */
int get_random_landclass(vec2 co, vec2 tile_size)
{
    float r = rand_2d(floor(vec2(co.s*tile_size.x, co.t*tile_size.y)/random_landclass_square_size_in_m));
...
//  * Note terrain default effect only has controls for the texture stretching
//  * dimensions for the base texture.
//  * Non-base textures use hardcoded stretching of the ground texture coords,
//  * which are in units of meters.
//  */
// vec2 get_ground_texture_scale(int lc)
// {
...
/*
 * Look up texture coordinates and stretching scale of ground textures for the
 * base texture.
 *
 * Note terrain default effect only has controls for the texture stretching
 * dimensions for the base texture. Non-base textures use hardcoded stretching
 * of the ground texture coords, which are in units of meters.
 */
void get_ground_texture_data(in int lc,
                             in vec2 tile_coord,
                             out vec2 st,
                             out vec2 g_texture_scale,
                             inout vec4 dFdx_and_dFdy)
{
    // Look up stretching dimensions of ground textures in m - scaled to
    // fit in [0..1], so rescale
    vec2 g_texture_stretch_dim = fg_dimensionsArray[lc].st;
 ...
 * type=0: use perlin texture, type=1: use noise_2d to avoid texture lookup
 *
 ...
 * (vectorising noise_2d versus just many texture calls).
 *
 * TODO: adjust for non-tile based ground coords.
 */
vec2 detile_texcoords_with_perlin_noise(in vec2 st,
                                        in vec2 ground_texture_scale,
                                        in vec2 tile_coord,
                                        inout vec4 dFdx_and_dFdy)
{
    ...
    // Note: unresolved texture discontinuties (i.e. mipmap problems) with
    // unequal stretch factors.
    ...
    if (detiling_noise_type==1) {
        ...
        pnoise[0] = noise_2d(st / (local_stretch_factors[0]), ptex_wavelength);
        pnoise[1] = noise_2d(-st / (local_stretch_factors[1]), ptex_wavelength);
    }

    if (pnoise[0] >= 0.5) {
        // TODO: fix once ground coords are no longer tile based
...
/*
 * Lookup a ground texture at a point based on the landclass at that point,
 * without visible seams at coordinate discontinuities or at landclass
 * boundaries where texture are switched. The partial derivatives of the
 * tile_coord at the fragment is needed to adjust for the stretching of
 * different textures, so that the correct mip-map level is looked up and there
 * are no seams.
 *
 * Texture types:
 *   - 0: base texture
...
{
    // Testing: may be able to save 1 or 2 op slots by combining dx/dy in a vec4
    // and using swizzles which are free, but mostly operations are working
    // independenly on s and t. Only 1 place so far that just multiplies
    // everything by a scalar.
    vec2 st;
    ...
    // Find the index of the specified texture type (e.g. mix or gradient
    // texture) in the ground texture lookup array.
    // Since texture_type is a constant in the fragment shader, there should
    // be no performance hit for branching.
    ...
    if (tex_lookup_type == 0) {
        ...
        texel = texture(textureArray, vec3(st, tex_idx));
    }
...
/*
 * Look up the texel of the specified texture type (e.g. grain or detail
 * textures) for this fragment and any neighbor texels, then mix.
 */
                          ...
                          in vec4 dFdx_and_dFdy)
{
    ...
    vec4 texel = ws30_lookup_ground_texture_array(0, st, lc, dxdy);
    ...
    if (num_unique_neighbors != 0) {
        ...
        vec4 texel_closest = ws30_lookup_ground_texture_array(0, st, lc_n[0], dxdy);
        ...
        if (num_unique_neighbors > 1) {
            ...
            vec4 texel_2nd_closest = ws30_lookup_ground_texture_array(0, st, lc_n[1], dxdy);
 ...
 */
float get_growth_priority(int current_landclass, int neighbor_landclass)
{
 ...
 * 1 = grow neighbor, 0 = don't grow neighbor
 */
float get_growth_priority(int current_landclass, int neighbor_landclass1, int neighbor_landclass2)
{
    int lc1 = current_landclass;
...
int lookup_landclass_id(in vec2 tile_coord,
                        in vec4 dFdx_and_dFdy,
                        out ivec4 neighbor_texel_landclass_ids,
                        out int number_of_unique_neighbors_found,
                        out vec4 landclass_neighbor_texel_weights)
{
    // TODO: fix landclass border artifacts, with all shaders.
         ...
         (enable_large_scale_transition_search == 0) ) )
    {
        ...
        if (remove_squareness_from_landclass_texture == 1) {
            ...
            if (lod_factor < cutoff) {
                ...
                float tn = noise_2d(c0*texture_dim_tx, wl_tn); // old val 1.6
                ...
                // TODO: Mix factor between different neighbour lanclasses
                           ...
                           out vec4 mix_factor)
{
        ...
        (use_landclass_texel_scale_transition_only == 0))
    {
        ...
        if (enable_dithering_for_large_scale_transitions == 1) {
            ...
            float tnoise1= noise_2d(tile_coord, dithering_noise_wavelength_as_fraction_of_step_size*steps.x);
        ...
        if (grow_landclass_borders_with_large_scale_transition == 1) {
            ...
            // Noise on the scale of landclass texels in the texture
            float tnoise2 = noise_2d(tile_coord, 0.4*transition_search_distance_in_m/tile_size.x);
                        ...
                        out vec2 st)
{
    ...
    st = texture_scaling.st * ground_texcoord.st;
